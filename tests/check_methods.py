...
def check_status_code_200(client, url):
    response = client.get(url)
    assert response.status_code == 200


def check_status_code_302(client, url):
    response = client.get(url)
    assert response.status_code == 302


def check_content_text_type(client, url):
    response = client.get(url)
    assert response.content_type == 'text/html; charset=utf-8'


def check_content_image_type(client, url):
    response = client.get(url)
    assert response.content_type == 'image/png'


def check_404(client, url):
    response = client.get(url)
    assert b'html' in response.data
    assert b'p class="none"' in response.data


def check_login_required(app, client, url):
...
def check_profile_auth_data(client, url):
    response = client.get(url)
    check_auth_menu(client, url)
    assert b'html' in response.data
